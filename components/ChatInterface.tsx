...
function getSessionId(): string {
  const stored = localStorage.getItem('terence-session-id');
  if (stored) return stored;
  const id = crypto.randomUUID();
  localStorage.setItem('terence-session-id', id);
  return id;
}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 select-none">
            <div className="w-16 h-16 rounded-full bg-[#008035] flex items-center justify-center text-white text-2xl font-bold shadow-lg">
              T
            </div>
            <div className="text-center">
              <p className="text-sm font-medium text-gray-700">Terence</p>
